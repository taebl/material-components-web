...
export default class MDCToolbarFoundation extends MDCFoundation {
  ...
  constructor(adapter) {
    ...
    this.scrollHandler_ = () => this.updateToolbarStyles();
    this.keyHeightsFrame_ = 0;
    this.scrollFrame_ = 0;
  ...
  init() {
    ...
    this.initKeyRatio_();
    this.setKeyHeights_();
    this.adapter_.addClass(MDCToolbarFoundation.cssClasses.FIXED_UPGRADE);
  ...
  setKeyHeights() {
    cancelAnimationFrame(this.keyHeightsFrame_);
    this.keyHeightsFrame_ = requestAnimationFrame(() => this.setKeyHeights_());
  }

  updateAdjustElementStyles() {
    if (this.fixedLastrow_ || this.fixedAllRow_) {
      this.adapter_.setStyleForFixedAdjustElement('marginTop', `${this.calculations_.toolbarHeight}px`);
    }
  }
